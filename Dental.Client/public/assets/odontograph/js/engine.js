...
function odontographMenuLabel(id, fallback) {
    if (typeof window !== 'undefined' && window.OdontographI18n && typeof window.OdontographI18n.label === 'function') {
        return window.OdontographI18n.label(id, fallback);
    }
    return fallback;
}

function odontographViewLabel(viewKey, fallback) {
    if (typeof window !== 'undefined' && window.OdontographI18n && typeof window.OdontographI18n.viewLabel === 'function') {
        return window.OdontographI18n.viewLabel(viewKey, fallback);
    }
    return fallback;
}
...
Engine.prototype.init = function () {
    ...
    this.useHtmlToolbar = (typeof window !== 'undefined' && window.OdontographUseHtmlToolbar === true);

    if (!this.useHtmlToolbar) {
        this.createMenu();

        this.adult = new MenuItem();
        this.adult.setUp(10, 150, 75, 20);
        this.adult.textBox.text = odontographViewLabel('adult', 'Adult');
        this.adult.active = true;
        this.buttons.push(this.adult);

        this.child = new MenuItem();
        this.child.setUp(90, 150, 75, 20);
        this.child.textBox.text = odontographViewLabel('child', 'Child');
        this.child.active = false;
        this.buttons.push(this.child);

        this.clear = new MenuItem();
        this.clear.setUp((this.canvas.width - 10) - 76, 150, 75, 20);
        this.clear.textBox.text = odontographViewLabel('reset', 'Reset');
        this.clear.active = false;
        this.buttons.push(this.clear);
    } else {
        this.menuItems = [];
        this.buttons = [];
        this.adult = { active: true, textBox: { text: '' }, rect: { checkCollision: function () { return false; } } };
        this.child = { active: false, textBox: { text: '' }, rect: { checkCollision: function () { return false; } } };
        this.clear = { active: false, textBox: { text: '' }, rect: { checkCollision: function () { return false; } } };
    }
...
Engine.prototype.load = function (tooth, damage, surface, note) {
    "use strict";
    if (tooth === undefined || tooth === null) {
        return;
    }

    surface = surface == null ? "0" : String(surface);
    ...
    if (surface === "0") {
        ...
        if (tooth < 1000) {
            ...
            var space = this.getSpaceById(tooth);

            if (!space) {
                return;
            }

            this.collisionHandler.handleCollision(space, damage);
        }
        ...
        // getData() stores full checkbox ids (e.g. "18_M"); avoid "18_18_M"
        var surfaceId = surface.indexOf("_") >= 0 ? surface : (tooth + "_" + surface);
        ...
        var surfaceObj = t.getSurfaceById(surfaceId);
        if (!surfaceObj) {
            return;
        }
...
Engine.prototype.createMenuButton = function (x, y, width, height, text, id) {
    ...
    menuitem.textBox.text = odontographMenuLabel(id, text);
    menuitem.id = id;
